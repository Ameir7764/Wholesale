...
export default function WholesalerPaymentProviders({ initialProviders }: { initialProviders: Provider[] }) {
  ...
  async function fetchProviders() {
    const res = await fetch(`/api/wholesaler/payment-providers`);
  ...
  async function handleAddOrUpdate(e?: React.FormEvent) {
    ...
    if (editingId) {
      await fetch(`/api/wholesaler/payment-providers`, {
        method: "PUT",
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/wholesaler/payment-providers?id=${encodeURIComponent(id)}`, { method: "DELETE" });
    await fetchProviders();
  }

  return (
    <div className="space-y-6 max-w-2xl">
      <form onSubmit={handleAddOrUpdate} className="grid grid-cols-1 gap-3">
        <div>
          <label className="block text-sm font-bold mb-1">اسم المحفظة / الموفر</label>
          <input value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 rounded-lg border" />
        </div>
        <div>
          <label className="block text-sm font-bold mb-1">رقم الحساب / رمز المحفظة</label>
          <input value={accountNumber} onChange={(e) => setAccountNumber(e.target.value)} className="w-full px-3 py-2 rounded-lg border" />
        </div>
        <div>
          <label className="block text-sm font-bold mb-1">تعليمات (مثلاً طريقة الدفع بالكود)</label>
          <textarea value={instructions} onChange={(e) => setInstructions(e.target.value)} className="w-full px-3 py-2 rounded-lg border" />
        </div>
        <div className="flex gap-2">
          <button type="submit" className="px-4 py-2 bg-blue-600 text-white rounded-lg">{editingId ? "حفظ" : "إضافة"}</button>
      ...
      <div>
        <h3 className="text-lg font-black mb-2">محافظ متجرك</h3>
        <div className="grid gap-3">
          {providers.map((p) => (
            <div key={p.id} className="p-3 border rounded-lg flex justify-between items-start">
              <div>
                <div className="font-bold">{p.name}</div>
                {p.accountNumber && <div className="text-sm text-slate-600">رقم: {p.accountNumber}</div>}
